...
<div class="col-md-8 col-lg-6 vstack gap-4">
    <div class="card">
        <div class="card-header border-0 pb-0">
            <div class="row g-2">
                <div class="col-lg-6">
                    <h1 class="h4 card-title mb-lg-0">VINS - Moneda Vinculamos</h1>
                </div>
                <div class="col-sm-3 col-lg-3 ms-lg-auto"></div>
                <div class="col-sm-3 col-lg-3">
                    <a class="btn btn-primary-soft ms-auto w-100" href="{{ route('vins.create') }}">
                        <i class="fa-solid fa-plus pe-1"></i> Nueva puntuación
                    </a>
                </div>
            </div>
            
            @if(Session::has('registrarPuntuacion'))
                <div class="alert alert-success alert-dismissible fade show mt-2 mb-0 text-center" role="alert">
                    <strong>{{ Session::get('registrarPuntuacion') }}</strong>
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Cerrar"></button>
                </div>
            @endif

            @if(Session::has('actualizarPuntuacion'))
                <div class="alert alert-success alert-dismissible fade show mt-2 mb-0 text-center" role="alert">
                    <strong>{{ Session::get('actualizarPuntuacion') }}</strong>
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Cerrar"></button>
                </div>
            @endif
                    ...
                    <div class="row g-4">
                        ...
                        @empty
                            <div class="alert alert-warning alert-dismissible fade show mt-2 mb-2 text-center" role="alert">
                                <strong>No existen puntuaciones asignadas.</strong>
                            </div>
                        @endforelse
                    </div>
                </div>
