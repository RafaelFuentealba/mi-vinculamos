...
<div class="col-md-8 col-lg-6 vstack gap-4">
    <div class="card">
        <div class="card-header border-0 pb-0">
            @if(isset($recurso))
                <h1 class="h4 card-title mb-4">Editar recurso</h1>
            @else
                <h1 class="h4 card-title mb-4">Crear recurso</h1>
            @endif
        </div>
        <div class="card-body">
            @if(isset($recurso))
                <form class="row g-3" action="{{ route('recursos.update', $recurso->trec_codigo) }}" method="POST">
                    @method('PUT')
                    @csrf
            @else
                <form class="row g-3" action="{{ route('recursos.store') }}" method="POST">
                    @csrf
            @endif

            @if(Session::has('errorRecurso'))
                <div class="alert alert-danger alert-dismissible fade show mt-2 mb-2 text-center" role="alert">
                    <strong>{{ Session::get('errorRecurso') }}</strong>
